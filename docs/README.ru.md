...
Библиотека реализующая ABAC(Attribute Based Access Control) и ABM(Attribute-Based Messaging) для серверов на Node.JS. 
...
* [Attribute-Based Messaging](https://www.youtube.com/watch?v=d1M6yMS25BA)

## Зачем

Необходим инструмент для изменения бизнес правил касаемо доступа и нотификации поьзователей, без внесения изменений в код сервера.
...
4. Позволяет динамически вычеслять список пользователей которых необходимо уведомить об изменении ресурса(чаты, пушнотификейшенс)
...
>  "target": [
>    	"action.type='approve'"
>		"user.position='top_manager'"
>		"user.department='sales'"
>		"user.approve_limit<user.current_approve"
>		"user.id!=resource.creator_id"
>		"user.branch=resource.branch"
>		"resource.type='purchase'"
>		"resource.cost>100000"
>  ]
> }
> ```
> Эти правила вы можете загрузить, откомпилировать в функцию и использовать в мидлваре для ограничения доступа. Кроме этого, так как в данные условия входят и условия для доступа к ресурсам(`resource`), возможно формирование фильтра для выборки ресурса из базы данных. Данный фильтр формируеться на базе JSONB структуры из входных данных, и может быть использованн для дальнейшей фильтрации в [Mongoose](http://mongoosejs.com/docs/queries.html), [Sequelize](http://docs.sequelizejs.com/manual/tutorial/querying.html#jsonb) или для написания кастомной логики. Но вы можете и сами переопредлить формат фильтра, используя кастомный адаптер.

## API
Библиотека оперирует с четырьмя сущностями:

* `user` - объект пользователя, предоставляет информацию по каждому уникальному пользователю.
* `action` - информация непосредственно о совершаемом действии.
* `env` - информация об окружении, содержащая как правило время и другую необходимую информацию.
* `resource` - объект с которым необходимо произвести действия.
...
Создает новую политику из правил. Во время создания политики, происходит компиляция правил в последовательность данных которые будет легче использовать во время проверки доступа и вычисления динамических фильтров.
...
* `target` - набор логических условий для вычисления политики, каждое условие возвращает `true`, `false` или ошибку в случае возникновения исключения. Состоит из трех частей: правого и левого операнда и [оператора](). Левый операнд должен быть объектом и его атрибутом к которому применяеться оператор. Правый операнд может быть константой, статическим объектом или атрибутом друго объекта. Атрибуты одно объекта как два разных операнда в одном выражении участвовать не могут. К операндам могут применяться [мутаторы](). И операторы и мутаторы могут быть переопределены.
...
* `effect` - эффект, который накладывают на результаты вычисления политики. Может принимать значения ***permit*** или ***deny***. По умолчанию - ***permit***. Если после вычисления правил и применения алгоритма, получено `true`, то накладывается эффект, и политика возвращает `true` если разрешено или `false` если запрещено, или в ходе вычисления политики произошла ошибка или использовано неизвестное значение.
...
Метод для вычисления значения политики, возвращает `true` `false` на основании вычисления правил с переданными параметрами, и применения к результату вычисления `effect`.
...
let user = {value: 4000};
policy.check({ user, env: {}, action: {}}) // <= true
```

> Обратите внимание на то что в метод `check` 
